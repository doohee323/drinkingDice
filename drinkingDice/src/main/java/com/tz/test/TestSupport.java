...
public abstract class TestSupport implements Test {
    ...
    public void given(String message, Object... args) {
        ...
        init();
        checkFirstCallAndUseRollMethodNameAsScenarioMessage();
    ...
    private void printFixtureInformation() {
        Method rollMethod = getRollMethod();
        if(rollMethod == null)
            return;
        Class<?> clazz = rollMethod.getDeclaringClass();
        println("[Start scenario set : %s]", clazz.getSimpleName());
        println("\ttime: %tc", System.currentTimeMillis());
    }

    private void checkFirstCallAndUseRollMethodNameAsScenarioMessage() {
        if (isNotFirstCall())
            return;

        String rollMethodName = getRollMethodName();
        if (rollMethodName != null)
            scenario(getRollMethodName());
    }
